...
from PyQt5.QtWidgets import (QMainWindow, QApplication, QWidget, QAction,
                             ...
                             QTabWidget, QGridLayout,QLineEdit, QFormLayout, QGroupBox, 
                             QVBoxLayout, QHBoxLayout, QLabel, QDialog, QHeaderView, QPushButton)
from PyQt5.QtGui import QIcon, QPixmap, QImage, QPalette, QBrush, QFont

df = pd.read_excel('kud_result5.xlsx',sheet_name = 'sheet3')
df = df[['Sl. No.', 'Subject', 'Pass', 'Fail', 'Total', 'Pass Percentage']]

class SubjectReportTable(QAbstractTableModel):
...
class SubjectReport(QWidget):
    def __init__(self):
        super(SubjectReport,self).__init__()
        self.setupUi

    def setupUi(self):
        self.subject_report_table_model = SubjectReportTable(df)
        self.view = QTableView()
        self.view.setModel(self.subject_report_table_model)
        self.view.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeToContents)
        self.view.setMaximumSize(490,233)
        ...
        self.headingLabel = QLabel('Subject Wise Report')
