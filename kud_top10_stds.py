...
df = pd.read_excel('kud_result5.xlsx',sheet_name = 'sheet4')
df = df[['Sl. No.', 'Seat No.', 'Name', 'Marks Obtained', 'Percentage']]

class Top10StdsTable(QAbstractTableModel):
...
class Top10Stds(QWidget):
    def __init__(self):
        super(Top10Stds,self).__init__()

    def setupUi(self):
        self.subject_report_table_model = Top10StdsTable(df)
        self.view = QTableView()
        self.view.setModel(self.subject_report_table_model)
        self.view.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeToContents)
        self.view.setMaximumSize(508,323)
        ...
        self.headingLabel = QLabel('Top 10 Students')
